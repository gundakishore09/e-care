...
public class compose extends HttpServlet
{
        ...
        throws ServletException, IOException
    {
        ...
        try
        {
            ...
            String s4 = l + "/" + s6 + "/" + i1;
            if(s.equals("level1"))
                i = st.executeUpdate("insert into level1messages values('" + s + "','" + s1 + "','" + s2 + "','" + s3 + "','" + s4 + "')");
            else
            if(s.equals("level2"))
            {
                System.out.println("second level admin");
                j = st.executeUpdate("insert into level2messages values('" + s + "','" + s1 + "','" + s2 + "','" + s3 + "','" + s4 + "')");
            } else
            if(s.equals("level3"))
                k = st.executeUpdate("insert into level3messages values('" + s + "','" + s1 + "','" + s2 + "','" + s3 + "','" + s4 + "')");
            else
            if(s.equals("all"))
            {
                i = st.executeUpdate("insert into level1messages values('" + s + "','" + s1 + "','" + s2 + "','" + s3 + "','" + s4 + "')");
